...
def resize_keep_aspect_ratio(image, width, height, interpolation=None):
    h, w = image.shape[:2]
    r0 = w / h
    r1 = width / height
    if r0 > r1:
        return resize(image, width=width, height=None, interpolation=interpolation)
    else:
        return resize(image, width=None, height=height, interpolation=interpolation)
